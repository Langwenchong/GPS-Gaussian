...
from lib.utils import export_ply


def pts2render(data, bg_color,export):
    ...
    for i in range(bs):
        ...
        if export:
            gaussian_cloud_i = export_ply(data,i,pts_xyz_i, pts_rgb_i, rot_i, scale_i, opacity_i)
            gaussian_cloud_list.append(gaussian_cloud_i)

    data['novel_view']['img_pred'] = torch.concat(render_novel_list, dim=0)
    return data,gaussian_cloud_list
