...
def export_ply(data, idx, pts_xyz, pts_rgb, rotations, scales, opacity):
    pts_xyz = pts_xyz - pts_xyz.median(dim=0).values
    scale_factor = pts_xyz.abs().quantile(0.95,dim=0).max()
    ...
    rotation = torch.tensor(rotation, dtype=torch.float32, device=pts_xyz.device)
    adjustment = torch.tensor(
        R.from_rotvec([0, 0, 0], True).as_matrix(),
        dtype=torch.float32,
        device=pts_xyz.device,
    )
    rotation = adjustment@rotation
    pts_xyz = einsum(rotation,pts_xyz,"i j,... j->... i")
    ...
    pts_SH = (pts_rgb-0.5)/C0

    dtype_full = [(attribute, "f4") for attribute in construct_list_of_attributes(45)]
    elements = np.empty(pts_xyz.shape[0], dtype=dtype_full)
    attributes=(
        pts_xyz.detach().cpu().numpy(),
        torch.zeros_like(pts_xyz).detach().cpu().numpy(),
        pts_SH.detach().cpu().numpy(),
        *[np.zeros((pts_xyz.shape[0],1), dtype="f4") for _ in range(45)],
